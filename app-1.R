...
ui <- page_navbar(
  ...
  nav_panel(
    ...
    card(
      card_body(plotOutput(outputId = "avg_salary_plot"))
    )
  ),
  nav_panel(
    title = "Individual salaries",

    # Task 3: Lay out the three components so slider is on top
    # and plot and data frame are next to each other

    # Task 4: Place each component into card()
    conditionalPanel(
